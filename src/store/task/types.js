export const taskTypes = {
    GET_TASKS_PENDING:          '[Task] Get Tasks pending',
    GET_TASKS_FULFILLED:        '[Task] Get Tasks success',
    GET_TASKS_REJECTED:         '[Task] Get Tasks failed',
    DELETE_TASK_PENDING:        '[Task] Delete Task pending',
    DELETE_TASK_FULFILLED:      '[Task] Delete Task success',
    DELETE_TASK_REJECTED:       '[Task] Delete Task failed',
    ADD_TASK_PENDING:           '[Task] Add Task pending',
    ADD_TASK_FULFILLED:         '[Task] Add Task success',
    ADD_TASK_REJECTED:          '[Task] Add Task failed',
    COMPLETE_TASK_PENDING:      '[Task] Complete Task pending',
    COMPLETE_TASK_FULFILLED:    '[Task] Complete Task success',
    COMPLETE_TASK_REJECTED:     '[Task] Complete Task failed'
};